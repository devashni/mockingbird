...
function makeMarkovText(chainsDict, minLength) {
    ...
    while (key in chainsDict) {
        ...
        //making a copy of chainsDict[key], so we can change it below without changing the original markov chainDict
        ...
        key = [key[1], nextWord];
        // We don't use 'nextWord' after this, so it is safe to change it with pop()
...
function returnStepsOfMarkovExecution(textString, minLength) {
    // e.g. steps = [['I'],['am'],['smart', 'dumb', 'crazy'],['person'],['and', 'but'],['cute.']]
...
// create div if there is just one element in steps[i]
function addTextDivAsChild(parentDivId = '#test-div', divText) {
    // Add code here to do the things.
    let newDiv = $('<div id="divID">' + divText +'</div>');
    newDiv.addClass('one-word');
    $(parentDivId).append(newDiv);
}

// //create unordered list if there are more than one element in steps[i]
// function  addTextListAsChild(parentUlId = '#test-ul', liText) {
//     // Add code here to do the things.
//     let newLi = $('<li>' + liText + '</li>').css("color", "red");
//     $(parentUlId).append(newLi);
// }
...
// This function animates the steps of a markov chain algorithm execution run.
// steps is a list/array of arrays. each element of steps is a 'step'
//      each 'step' is a a list/array of Strings
//      if the 'step' in the algorithm was just adding a word from a chain with only one word
//      then, the 'step' will be a list/array with only ONE string in it.
//      however, if the markov algorithm chose from multiple words, then the 'step' will
//      be a list/array of MULTIPLE strings.
// Currently this function ignores all the extra strings in a step and only uses ONLY the first
// string, as if it were just a single word step.
function animateStepsOfMarkovChain(parentDivId, steps) {
    ...
    let timer = setInterval(function() {
        // Add code here to 'output'/'show' the steps of the markov chain execution.
...
$(function () {
    ...
    let steps = returnStepsOfMarkovExecution(sourceText, 10);
    console.log(steps);
    animateStepsOfMarkovChain('#test-div', steps);
